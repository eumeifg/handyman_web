...
Route::get('/healthz', function () {
    // Perform health check (e.g., check if the application can handle requests)
    return response()->json(['status' => 'ok']);
});

Route::get('/ready', function () {
    // Perform readiness check (e.g., check database connection)
    try {
        DB::connection()->getPdo();
        return response()->json(['status' => 'ready']);
    } catch (\Exception $e) {
        return response()->json(['status' => 'not ready'], 500);
    }
});
