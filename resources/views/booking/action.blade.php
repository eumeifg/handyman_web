...
@if(!$booking->trashed())
    ...
    <a href="{{ route('booking.edit',$booking->id) }}" title="{{ __('messages.update_form_title',['form' => __('messages.booking') ]) }}"><i class="fas fa-pen text-secondary"></i></a>
    @endif

    @if($auth_user->can('booking view'))
    <a class="mx-2" href="{{ route('booking.show',$booking->id) }}" title="{{ __('messages.view_form_title',['form'=>  __('messages.booking') ]) }}"><i class="far fa-eye text-secondary"></i></a>
    @endif
