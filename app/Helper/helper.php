    ...
    if($file) {
        ...
        if (is_array($file)){
            foreach ($file as $key => $value){
                $model->addMedia($value)->toMediaCollection($name,env('FILESYSTEM_DRIVER'));
            }
        }else{
            $model->addMedia($file)->toMediaCollection($name,env('FILESYSTEM_DRIVER'));
        }
    }
